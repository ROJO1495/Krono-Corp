...
// ==============================================================
// VALIDACIÓN Y ENTRADA AL FLUJO (SOLO ROL EMPLEADO)
// ==============================================================
fun negocioPropio(usuario: User) {
    // Validación estricta de rol
    if (!usuario.role.equals("EMPLEADO", ignoreCase = true)) {
        println("\n❌ Acceso denegado: Este módulo solo está habilitado para usuarios con rol EMPLEADO.")
...
fun home(usuario: User, negocio: Negocio, listaNegocios: MutableList<Negocio>) {
    while (true) {
        ...
        println("          Bienvenido, ${if (usuario.name.isNotEmpty()) usuario.name else usuario.email}")
        ...
        when (readln().trim().uppercase()) {
            "1" -> {
                ...
                if (nom.isNotEmpty()) {
                    ...
                    val nuevoId = "SUC-${negocio.sucursales.size + 1}"
...
fun menuConfiguracion(usuario: User, negocio: Negocio, listaNegocios: MutableList<Negocio>): Boolean {
    while (true) {
        ...
        when (readln().trim()) {
            "1" -> {
                println("\n--- MI CUENTA ---")
                println("Nombre: ${if (usuario.name.isNotEmpty()) usuario.name else "No especificado"}")
