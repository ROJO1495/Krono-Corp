...
import Login.User
// Se reutilizan el archivo y la lista de usuarios de login.kt para que ambos modulos vean los mismos datos
import Login.archivo as archivoUsuarios
import Login.listaUsuarios as usuariosEnMemoria
...
fun nuevaCuentaAdministracion(usuario: User) {
    ...
    // El login tambien acepta el nombre de usuario para iniciar sesion, y registro() no permite
    // alias repetidos, asi que el nombre debe ser unico entre TODOS los usuarios
    if (usuarios.any { it.alias.equals(nombre, ignoreCase = true) }) {
        println("Ya existe un usuario con el nombre \"$nombre\". Elige otro. No se creo la cuenta.")
        return
    }
    ...
    try {
        ...
        // Se agrega tambien a la lista en memoria de login.kt; si no, el siguiente guardado
        // hecho desde login o admin sobrescribiria usuario.json sin esta cuenta
        if (usuariosEnMemoria.none { it.alias.equals(nuevaCuenta.alias, ignoreCase = true) }) {
            usuariosEnMemoria.add(nuevaCuenta)
        }
